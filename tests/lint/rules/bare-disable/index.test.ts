...
test("asks a whole-file directive for its rules, or for the spelling that means every one", async () => {
  expect(await lint("whole-file")).toEqual([
    "whole-file.ts:1:1  bare-disable  valof-lint-disable-whole-file names no rule; name the ones it silences, or valof-lint-disable-all-whole-file for every one",
