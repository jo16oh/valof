...
describe("unused companion members", () => {
  test("reports a member nothing reads, and spares the one that is read", () => {
    expect(lint("unused/dead-member").findings).toEqual([
      "a.ts:3  unused-member  Id.shout is never read",
    ]);
  });

  test("finds the reader in another file, through a renamed import", () => {
    expect(lint("unused/renamed-import").findings).toEqual([
      "user.ts:3  unused-member  User.shout is never read",
    ]);
  });
  ...
  test("follows a read of a companion that was renamed on the way out", () => {
    expect(lint("unused/re-export").findings).toEqual([
      "user.ts:3  unused-member  User.shout is never read",
    ]);
  });
  ...
  test("collects method shorthand and non-function members", () => {
    expect(lint("unused/shorthand-and-const").findings).toEqual([
      "a.ts:2  unused-member  User.slug is never read",
      "a.ts:5  unused-member  User.MAX is never read",
    ]);
  });
  ...
  test("reads through a builder chain, past implSeal, implEquals and fixed", () => {
    expect(lint("unused/builder-chain").findings).toEqual([
      "a.ts:7  unused-member  Doc.subtitle is never read",
    ]);
  });

  test("keeps both declarations when two modules name a companion alike", () => {
    expect(lint("unused/same-name").findings).toEqual([
      "a.ts:1  unused-member  User.greet is never read",
      "b.ts:1  unused-member  User.greet is never read",
    ]);
  });
...
describe("what name resolution cannot reach", () => {
  test("reports a member that is only ever read through a computed key", () => {
    expect(lint("unused/computed-key").findings).toEqual([
      "a.ts:1  unused-member  User.greet is never read",
    ]);
  });
...
describe("duplicate brands", () => {
  test("reports every alias that claims a brand another one claims", () => {
    expect(lint("brand/duplicate").findings).toEqual([
      'billing.ts:1  duplicate-brand  Id claims the brand "Id", and so does another type',
      'orders.ts:1  duplicate-brand  OrderId claims the brand "Id", and so does another type',
    ]);
  });
  ...
  test("finds the collision when Val is renamed, or imported for its type alone", () => {
    expect(lint("brand/renamed-val").findings).toEqual([
      'billing.ts:3  duplicate-brand  Id claims the brand "Id", and so does another type',
      'orders.ts:3  duplicate-brand  OrderId claims the brand "Id", and so does another type',
    ]);
  });
...
describe("a child's own equals", () => {
  test("reports a parent that structurally compares a child carrying its own equality", () => {
    expect(lint("equals/plain").findings).toEqual([
      "order.ts:6  structural-equals  Order.total holds Money, which has its own equals",
    ]);
  });
  ...
  test("names the path through a plain nested object", () => {
    expect(lint("equals/nested").findings).toEqual([
      "order.ts:6  structural-equals  Order.shipping.fee holds Money, which has its own equals",
    ]);
  });

  test("names the element position of an array", () => {
    expect(lint("equals/array").findings).toEqual([
      "order.ts:6  structural-equals  Order.charges[] holds Money, which has its own equals",
    ]);
  });

  test("reports every level at once, so fixing one does not uncover another", () => {
    expect(lint("equals/cascade").findings).toEqual([
      "line.ts:6  structural-equals  OrderLine.total holds Money, which has its own equals",
      "order.ts:6  structural-equals  Order.lines[] holds OrderLine, which has its own equals",
    ]);
  });
...
describe("ignore comments", () => {
  test("silences every kind when the directive lists none", () => {
    expect(lint("ignore/whole-line").findings).toEqual([
      "a.ts:4  unused-member  User.whisper is never read",
    ]);
  });
  ...
  test("leaves a finding of another kind alone", () => {
    expect(lint("ignore/wrong-kind").findings).toEqual([
      "a.ts:3  unused-member  User.shout is never read",
    ]);
  });
  ...
  test("stops at a blank line, which starts a block of its own", () => {
    expect(lint("ignore/not-a-block").findings).toEqual([
      "a.ts:4  unused-member  User.shout is never read",
    ]);
  });

  test("ignores a directive trailing code, which belongs to no block", () => {
    expect(lint("ignore/after-code").findings).toEqual([
      "a.ts:2  unused-member  User.shout is never read",
    ]);
  });
});
...
describe("turning a rule off", () => {
  test("leaves out the kind the flag names, and only that one", () => {
    expect(lint("mixed").findings).toEqual([
      'a.ts:3  duplicate-brand  Id claims the brand "Id", and so does another type',
      "a.ts:6  unused-member  User.shout is never read",
      'b.ts:3  duplicate-brand  UserId claims the brand "Id", and so does another type',
    ]);
