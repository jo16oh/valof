...
（oxlint、JS プラグイン）。`always` が守るのは CLI のフラグだけである。

**この非対称は残す。**理由は 2 つ。

1. **プラグインには塞ぐ口が無い。**重大度はホストの機構で、規則を必須にする API は ESLint にも oxlint にも
   無い。1 つの規則として登録している以上 off にできる。避けるには 2 つを他の規則の report に混ぜること
   になり、規則ごとに重複した finding が出る
2. **設定ファイルはコミットされる。**`-all-whole-file` を逃げ道として認めた基準（そこに書いてあり grep
   できる）を満たす。1 回の run に付けるフラグとはそこが違う

README の Disable comments の節に、`--no-` を拒むことと、ホストでは重大度を設定できることを併記した。
